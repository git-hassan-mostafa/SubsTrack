...
export abstract class OfflineBaseRepository {
        ...
        if (before) {
          await this.auditIn(db, {
            ...
            branchId: branchColumn
              ? ((before[branchColumn] as string | null) ?? null)
              : null,
          });
        }
      }
  ...
  protected combineWhere(parts: { clause: string; params: unknown[] }[]): {
    sql: string;
    params: unknown[];
  } {
  ...
  protected all<T = Record<string, unknown>>(
    sql: string,
    params: unknown[] = [],
  ): Promise<T[]> {
  ...
  protected async customerAudit(
    ...
    const row = await this.first<{
      ...
      name: string | null;
    }>("SELECT branch_id, name FROM customers WHERE id = ?", [customerId]);
    return {
  ...
  protected async rowsById<T>(
    ...
    const rows = await this.all(
      `SELECT * FROM ${table} WHERE id IN (${ph})`,
      unique,
    );
    for (const raw of rows) {
