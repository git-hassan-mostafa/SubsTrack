...
export function CurrencyFormSheet({ currency, onDismiss, onRequestDelete }: Props) {
  const { t } = useTranslation();
  const { user } = useAuth();
  const { createCurrency, updateCurrency, reactivateCurrency, loading, error, clearError } =
    useCurrencyStore();
      ...
      <View className="flex-1 bg-white">
        ...
        <View className="flex-row items-center justify-between px-6 py-3 border-b border-gray-100">
          <Text fontWeight="Bold" className="text-lg text-gray-900">
            {currency ? t('tenant_settings.edit_currency') : t('tenant_settings.add_currency')}
          </Text>
        ...
        <ScrollView className="flex-1 px-6 pt-6" keyboardShouldPersistTaps="handled">
          ...
          <Button
            label={currency ? t('common.save_changes') : t('tenant_settings.add_currency')}
