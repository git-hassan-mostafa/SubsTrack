...
type PlanRow = { key: string; id?: string; planId: string | null; startDate: string };
...
export const CustomerPlansEditor = forwardRef<CustomerPlansEditorHandle, Props>(
  function CustomerPlansEditor({ customer, branchId, startDate }, ref) {
    ...
    function setRowStartDate(key: string, date: string) {
      setRows((prev) => prev.map((r) => (r.key === key ? { ...r, startDate: date } : r)));
    }
