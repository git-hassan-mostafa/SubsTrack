  ...
  implements ICurrencyRepository
{
  ...
  async countReferences(id: string): Promise<number> {
    const [plans, charges, collections, lines] = await Promise.all([
      ...
      this.count("SELECT COUNT(*) AS n FROM charges WHERE currency_id = ?", [
        id,
      ]),
      this.count(
        "SELECT COUNT(*) AS n FROM collections WHERE currency_id = ?",
        [id],
      ),
      this.count(
        "SELECT COUNT(*) AS n FROM customer_plans WHERE custom_currency_id = ?",
        [id],
      ),
    ]);
