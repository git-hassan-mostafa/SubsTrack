 ...
 */
export class OfflineAuditRepository
  ...
  implements IAuditRepository
{
  ...
  private where(
    ...
    if (filter.table)
      parts.push({ clause: "table_name = ?", params: [filter.table] });
    if (filter.action)
      parts.push({ clause: "action = ?", params: [filter.action] });
    if (filter.actorUserId)
      parts.push({ clause: "actor_user_id = ?", params: [filter.actorUserId] });
    if (filter.from)
      parts.push({ clause: "occurred_at >= ?", params: [filter.from] });
    if (filter.to)
      parts.push({ clause: "occurred_at <= ?", params: [filter.to] });
    parts.push(
      this.branchWhere(
  ...
  async findRecent(filter: AuditFilter, page = 0): Promise<AuditPage> {
    if (await isOnline()) {
      try {
        ...
        return {
          ...server,
          rows: OfflineAuditRepository.merge(server.rows, pending),
        };
      } catch {}
    }
    const { sql, params } = this.where(filter);
  ...
  private async timeline(
    ...
    if (await isOnline()) {
      try {
        ...
        const pending = await this.localRows(
          `WHERE (${clause}) AND _dirty = 1`,
          params,
        );
        return {
          ...server,
    ...
    return {
      rows: await this.localRows(`WHERE (${clause})`, params),
      source: "local",
    };
  }
  ...
  findForRecords(targets: AuditRecordTarget[]): Promise<AuditRows> {
    if (targets.length === 0)
      return Promise.resolve({ rows: [], source: "server" });
    return this.timeline(
