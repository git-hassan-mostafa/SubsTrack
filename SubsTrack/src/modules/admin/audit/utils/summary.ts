...
function recordPhrase(
  ...
  const named = !detail.text
    ? ctx.t(
        fresh ? "audit.summary.record.bare_new" : "audit.summary.record.bare",
        { type },
      )
  ...
  return ctx.t("audit.summary.record.with_plan", {
    record: named,
    plan: bold(plan),
  });
}
...
function fieldsList(changes: AuditChange[], ctx: AuditFieldContext): string {
  ...
  if (labels.length === 2) {
    return ctx.t("audit.summary.fields_two", {
      first: labels[0],
      second: labels[1],
    });
  }
...
function specialSentence(
  ...
  if (entry.table === "skipped_months" && detail.text) {
    ...
    const off =
      flip === false || (flip === undefined && entry.action === "restore");
  ...
  if (entry.table === "customer_plans" && detail.text) {
    ...
    const removed =
      entry.action === "delete" || (cancelled && !isBlank(cancelled.after));
    if (removed) {
      return t("audit.summary.special.unsubscribed", {
        actor,
        detail: bold(detail.text),
      });
    }
    if (entry.action === "create" || (cancelled && isBlank(cancelled.after))) {
      return t("audit.summary.special.subscribed", {
        actor,
        detail: bold(detail.text),
      });
    }
  ...
  if (
    ...
    detail.field &&
    entry.changes[0].field === detail.field
  ) {
    const c = entry.changes[0];
...
export function buildAuditSummary(
  ...
  const sentence =
    specialSentence(entry, ctx, phrases) ??
    genericSentence(entry, ctx, phrases);
  const subject =
    options.showSubject !== false && entry.table !== "customers"
      ? entry.subject
      : null;
  ...
  return toParts(
    ctx.t(`audit.summary.${key}`, { sentence, subject: bold(subject) }),
  );
}
