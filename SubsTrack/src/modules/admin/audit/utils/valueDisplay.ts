...
function idRef(
  ...
  return (value, { t, lookups }) => {
    ...
    return (
      lookups[kind](String(value)) ?? t(opts.missing ?? "audit.deleted_record")
    );
  };
}
...
function money(currencyColumn: string): AuditValueFormatter {
  return (value, { row, lookups }) => {
    ...
    const c = lookups.currencyObject(
      (row[currencyColumn] as string | null) ?? null,
    );
...
const DISPLAY: Record<string, AuditValueFormatter> = {
  ...
  "products.branch_id": idRef("branch", {
    blank: "branches.shared_all_branches",
  }),
  ...
  "tenant_settings.value": (value, ctx) =>
    setting(ctx.row.key)?.value(value, ctx) ?? null,
};
