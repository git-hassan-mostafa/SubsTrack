...
export function TierCard({
  ...
  const features: { key: string; label: string; value: string }[] = [
    {
      key: "customers",
      label: t("subscription.resource.customers"),
      value: formatLimit(tier.maxCustomers),
    },
    {
      key: "users",
      label: t("subscription.resource.users"),
      value: formatLimit(tier.maxUsers),
    },
    {
      key: "plans",
      label: t("subscription.resource.plans"),
      value: formatLimit(tier.maxPlans),
    },
    {
      key: "branches",
      label: t("subscription.resource.branches"),
      value: formatLimit(tier.maxBranches),
    },
    {
      key: "products",
      label: t("subscription.resource.products"),
      value: formatLimit(tier.maxProducts),
    },
    {
      key: "multi_currency",
  ...
  return (
    ...
    >
      ...
      {/* Upgrades are gated: when self-serve upgrade is disabled, the upgrade
          button falls back to a WhatsApp "contact to upgrade" button.
          Downgrades are never gated. */}
      {!isCurrent && onAction ? (
        direction === "upgrade" ? (
          <CanUpgrade
            fallback={
              <ContactToUpgradeButton tierName={tier.name} className="mt-4" />
            }
          >
            {actionButton}
          </CanUpgrade>
