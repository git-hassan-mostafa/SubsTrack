...
// The revenue trend spans every month of the current calendar year (Jan → Dec).
...
class DashboardService {
  async getMetrics(branchFilter: BranchFilter = null): Promise<DashboardMetrics> {
    ...
    const trendStartBilling = toBillingMonth(trendPoints[0].year, trendPoints[0].month);
    const trendEndBilling = toBillingMonth(trendPoints[MONTHS_IN_YEAR - 1].year, trendPoints[MONTHS_IN_YEAR - 1].month);
    ...
    const [
      ...
      paymentRepo.paidAmountsInRange(trendStartBilling, trendEndBilling, branchFilter),
    ...
    for (const p of trendPoints) buckets.set(monthKey(p.year, p.month), { subscription: 0, sales: 0 });
    ...
    const prevBucket = buckets.get(monthKey(prev.getFullYear(), prev.getMonth() + 1));
    const prevMonthRevenue = prevBucket ? prevBucket.subscription + prevBucket.sales : 0;
