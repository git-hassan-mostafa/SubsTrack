...
export function RevenueTrendChart({
  ...
  // Swiping the card pages the window, with the same guards as the chevrons. The
  // hook frames the flick as forward/back in reading order, so RTL is automatic.
  const swipeNext = useCallback(() => {
    if (!loading && !nextDisabled) onNext?.();
  }, [loading, nextDisabled, onNext]);
  const swipePrev = useCallback(() => {
    if (!loading) onPrev?.();
  }, [loading, onPrev]);
  const swipe = useHorizontalSwipe({ onNext: swipeNext, onPrev: swipePrev });
