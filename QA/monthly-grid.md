...
| 4.5.18  | Unskip locked by a later paid month    | Skip January, pay February, open the 3-dot menu on January                                    | **No "Unskip month"** row. Instead: Open, plus Pay now / Pay & send on WhatsApp for a fixed-price plan                                                                                                                                           |
| 4.5.19  | Tap a locked skipped cell              | Tap January's cell body (February paid)                                                       | The **payment form** opens (not the unskip dialog), with an amber note: skipped, can no longer be unskipped, paying settles it                                                                                                                   |
| 4.5.20  | Pay a locked skipped month             | Quick pay January from the menu                                                               | Payment recorded; the cell turns green ("Paid") — money outranks the skip; the skip row is untouched in the DB                                                                                                                                   |
| 4.5.21  | Undo that payment                      | **Void this month** on January                                                                | Blocked while February is paid (void newest-first, naming February); void February's month first, then January reverts to **skipped**. Voiding the single hand-over from **View bill** is not gated and reverts January to skipped straight away |
| 4.5.22  | Bulk unskip drops locked months        | Select a locked skipped month + an unlocked one → Unskip                                      | Only the unlocked month is offered/changed; the locked one appears under **Pay** instead                                                                                                                                                         |
| 4.5.23  | Multi-month block over a locked skip   | 3-month plan, month 1 of the window is a locked skip → Pay                                    | Allowed (no "months are skipped" error) — the block covers it and settles the month                                                                                                                                                              |
| 4.5.24  | Deep-link quick pay, locked skip       | Customer list quick-pay, current month skipped but a later month paid                         | The payment form opens instead of the "This month is skipped…" popup                                                                                                                                                                             |
...
| 11.9  | **Two voids, and the cell has only the wide one** | 3-dot on a paid month                                                                      | **Void this month** is listed (the BILL and the cash on it). Voiding a single **hand-over** is not here — it lives in **View bill**, because one payment can cover several bills (gotcha #109) |
| 11.9b | **Void this month is newest-first**               | 3-dot on a paid July while August is paid                                                  | A "Not available" popup names **August** ("Newer months must be voided first"); the destructive confirm never opens. Void August first, then July goes through                                 |
| 11.9c | **The month void asks for a REASON**              | 3-dot on a paid month → Void this month                                                    | A confirm dialog with a reason box, the shared-bill warning and an error banner — the same shape a sale's void wears. The reason is optional and is stored on the bill                         |
| 11.9d | One void UI across both kinds                     | Compare this dialog with a sale's void dialog                                              | Same layout, same reason box, same shared-bill warning. The month one names no count (it is always one bill)                                                                                   |
| 11.9e | The order gate still runs FIRST                   | 3-dot on a paid July while August is paid                                                  | The "Not available" popup appears and the reason dialog never opens — the gate is checked before the dialog, not inside it                                                                     |
| 11.10 | Collect the rest                                  | 3-dot on a partly-paid month                                                               | **Collect the rest** appears, pre-filled with the balance                                                                                                                                      |
| 11.11 | Void offered whenever a bill exists               | 3-dot on an unpaid month that still holds an emptied bill, then on one with no bill at all | Listed for the first (the bill holds the frozen price, so removing it is a real decision); NOT listed for the second — there is nothing to void                                                |
| 11.12 | **A written-off month opens its bill**            | Write off a month bill (from Debts), then tap that cell or its 3-dot                       | **BillSheet** opens — a written-off month with nothing collected is `unpaid` by the grid's money rule, but it HAS a bill, so it must never fall through to the collect sheet                   |
| 11.13 | The sheet says it plainly                         | That sheet                                                                                 | An **orange** hero card with a ✕ badge, the amount in orange, a "Written off" pill, and Written off on / by / reason rows — the same shape the sale receipt uses                               |
| 11.14 | No collecting while written off                   | The sheet, and the cell's 3-dot                                                            | No **Collect the rest** in either place; only **Undo write-off**. Reverting it first puts both back                                                                                            |
| 11.15 | A PART-PAID month that is written off             | Write off a month that had 5 of 20 collected                                               | The cell stays **paid** (money outranks a write-off, §7 rule 2) with its amber PARTIAL ring; the sheet reads "5.00 $ collected before it was written off" instead of a Remaining line          |
| 11.16 | **A write-off invents no cell colour**            | Compare a written-off month with a never-touched one                                       | Identical cells — there is no `written_off` MonthStatus, and the grid still keys purely off money                                                                                              |
...
| 11.15 | Skip / Unskip rows                                | 3-dot on an unpaid, a future, and a skipped cell                                           | "Skip month" listed on unpaid + future; "Unskip month" on skipped (only when no later month is paid); never both at once                                                                       |
...
| 12.18 | Bulk unskip                            | Select several skipped months → Unskip                        | All revert in one write; a month locked by a later payment is not among them (see 4.5.22)                           |
...
| 13.10b | Backlog in a FUTURE year (never paid into)     | Line starts 2026, nothing paid in 2027 → navigate to 2027 and tap February | Blocked; the popup names **January 2027**. The uncovered walk must reach the VIEWED year, not stop at the last paid one (gotcha #122) |
| 13.10c | Prepay in order inside a future year           | In 2027, pay January → tap February                                        | Allowed; February opens once January is covered                                                                                       |
...
## 14. Partial payment is visible in the grid and the receipt

A partial payment still resolves to `"paid"` (rule #1) — the ring and the label are the only things that separate a settled month from one that still owes.

| #     | Scenario                          | Steps                                                           | Expected result                                                                                                                                             |
| ----- | --------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 14.1  | Partial cell is marked            | Pay January partially (e.g. 20 of 50) → look at the grid        | January keeps its **paid** fill (green for regular, yellow for non-regular) but gains an **amber ring**, and the sublabel reads `PARTIAL` instead of `Paid` |
| 14.2  | Fully paid is unchanged           | Pay February in full                                            | Plain green (or yellow) cell, no ring, sublabel `Paid` — exactly as before this change                                                                      |
| 14.3  | Non-regular customer              | Repeat 14.1 on a non-regular customer                           | Yellow fill + amber ring; the ring is what makes it readable, since yellow and amber fills are nearly the same colour                                       |
| 14.4  | Multi-month partial block         | Record a 3-month payment, paying only part of it                | Only the block's **first** cell wears the ring and says `PARTIAL`; months 2–3 stay `INCL.` with no ring, so the joined pill keeps no internal seams         |
| 14.5  | Receipt shows collected over owed | Tap the partial month → the receipt sheet                       | The amber hero reads **`20/50 $`** (collected/owed) instead of just the collected amount; the currency label appears **once**                               |
| 14.6  | Full receipt unchanged            | Tap a fully-paid month                                          | Green hero still shows the single amount — no fraction (nothing is owed)                                                                                    |
| 14.7  | Detail rows still agree           | On the partial receipt, read Amount Due / Amount Paid / Balance | Unchanged from before: the three rows carry the same figures the hero fraction summarises                                                                   |
