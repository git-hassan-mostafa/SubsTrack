...
| 1.5b | Restock a product     | Product menu → Adjust Stock → Add 10 → Save                  | **Nothing.** A stock entry is its own create record — only an _edit_ or a _revert_ of one is audited                                                                                         |
...
The card is deliberately two lines: **record type + the customer it belongs to + the action pill**, then **staff · when**. Naming _what_ moved is the detail sheet's job. The customer name comes from `audit_logs.subject`, **frozen at write time** — never resolved from `customer_id` at read time, because a deleted customer leaves the id pointing at nothing.
...
| 6.2  | Create sheet             | Tap an **Added** entry                                                               | The whole new row's values; no arrows (nothing changed _from_)                                                                            |
...
## 9. The entry reads as a sentence

Each row is one line of prose built at read time (`buildAuditSummary`). Bold marks the actor, the record, the customer and the new value. See gotcha #132.

| #    | Scenario                       | Steps                                                                               | Expected result                                                                                                                                   |
| ---- | ------------------------------ | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 9.1  | A create names the record      | Add a customer → open the Audit Log                                                 | "_Name_ added a new customer **John Doe**" — no record-type line, no subject pill, timestamp on its own muted line                                |
| 9.2  | A month void names the month   | Void a paid month → open the Audit Log                                              | "_Name_ voided the **March 2026** bill for **John Doe**" — the month, not `2026-03-01`, and not the bare word "Bill"                              |
| 9.3  | One field carries old → new    | Change only a plan's price                                                          | "… changed Price on the plan **Gold** from 10.00 $ to **12.00 $**" — money in the currency stored, never re-converted                             |
| 9.4  | Many fields collapse           | Change five fields on one plan in one save                                          | "… updated Price, Name and 3 other fields on the plan **Gold**" — exactly two named, the rest counted                                             |
| 9.5  | Two fields are joined          | Change exactly two fields                                                           | "… updated Price and Months covered on …" — joined with "and", never "and 0 other fields"                                                         |
| 9.6  | A flag is spoken, not diffed   | Deactivate a staff member, then reactivate                                          | "deactivated the staff member **Ali Hassan**" / "activated …" — never "changed Active from Yes to No"                                             |
| 9.7  | A customer cancel reads right  | Deactivate a customer                                                               | "cancelled the customer **John Doe**"; reactivating reads "reinstated"                                                                            |
| 9.8  | A rename is a rename           | Change **only** a plan's name from Silver to Gold                                   | "renamed the plan Silver to **Gold**" — never "changed Name on the plan **Gold** from Silver to **Gold**"                                         |
| 9.9  | Write-off ≠ void               | Write off a bill                                                                    | "wrote off the **March 2026** bill for **John Doe**", distinct from a void; undoing it reads "undid the write-off"                                |
| 9.9b | **Undo ≠ a bill taking money** | Undo a write-off from Debts (no cash involved), then compare with a bill that was written off and later COLLECTED | The undo reads "undid the write-off on …"; the collected one reads "re-opened …". The difference is `issued_at` — only money re-raises the bill  |
| 9.10 | A skip reads as a skip         | Skip a month, then unskip it                                                        | "skipped **March 2026** for **John Doe**" / "removed the skip on …" — not "added a new skipped month"                                             |
| 9.11 | A sale is its receipt number   | Void a sale                                                                         | "voided the **#0AB12C** sale for **John Doe**" — matches the receipt number on the sale card                                                      |
| 9.22 | Cash is recorded, not added    | Record a payment, then void it                                                      | "recorded a payment of **20.00 $** from **John Doe**" / "voided a payment of **30.00 $** from **John Doe**" — "from", not "for"                   |
| 9.23 | No raw label ever leaks        | Scroll to entries written before this change                                        | No `·`-joined dumps, no raw `2026-07-01` or ISO timestamps; they read "voided **a bill**" / "voided **a payment**"                                |
| 9.24 | Articles are never wrong       | Any entry with and without a name                                                   | "added **a new** customer X" / "deleted **a** plan" / "voided **the** March 2026 bill" — never "voided the bill"                                  |
| 9.12 | A stock edit names its product | Edit a manual stock entry's quantity                                                | "changed Quantity on a stock entry from 5 to 8 for **Widget A**" — the product is the subject                                                     |
| 9.13 | A setting reads plainly        | Change the unpaid-months rule                                                       | "set Unpaid months rule to **…**" — never "changed Unpaid months rule on the setting Unpaid months rule"                                          |
| 9.14 | A password is never shown      | Change a staff member's password                                                    | "changed the password for the staff member **Ali Hassan**" — no value, masked or otherwise                                                        |
| 9.15 | An old row still reads         | Scroll to entries written before this change (or an update with no context columns) | The record falls back to "a bill" / "a payment"; the sentence is thinner but never blank and never ends mid-phrase                                |
| 9.16 | No repeated customer name      | Open **Customer history** and **Bill history**                                      | No "for John Doe" on every row (the header already says it); the record detail is still there                                                     |
| 9.17 | The global log keeps it        | Open Admin → Audit Log                                                              | "for **John Doe**" IS present — this list has no header naming the customer                                                                       |
| 9.18 | Arabic sentence                | Switch to Arabic → repeat 9.1–9.5                                                   | Sentences read in Arabic word order, bold lands on the same nouns, nothing reversed or split mid-word                                             |
| 9.19 | Long sentence wraps            | An entry with a long customer name and a long value                                 | The card wraps to at most 3 lines and truncates; the sheet shows it in full                                                                       |
| 9.20 | The sheet leads with it        | Tap any row                                                                         | The same sentence sits at the top of the sheet; the meta / diff / snapshot cards below are unchanged                                              |
| 9.21 | No stray marker glyphs         | Any entry whose customer name or description contains unusual characters            | No boxes, question marks or private-use glyphs anywhere in the sentence                                                                           |
| 9.25 | A re-opened bill               | Collect money on a month bill that was voided earlier                               | "re-opened the **September 2026** bill for **John Doe**" — never "updated issued_at, Voided at and 1 other field"                                 |
| 9.26 | A written-off bill re-opens    | Collect money on a bill that was written off                                        | Also reads "re-opened …", and the payment gets its own card beside it                                                                             |
| 9.27 | A real void still voids        | Void a bill that was never voided before                                            | "voided the **March 2026** bill …" — the re-open wording must NOT appear                                                                          |
| 9.28 | Regular-customer mark          | Turn "Regular customer" on for a customer, then off                                 | "marked the customer **John Doe** as a regular customer" / "removed the regular customer mark from …"                                             |
| 9.29 | Per-customer price flag        | Turn a plan's custom-price flag on, then off                                        | "turned on a per-customer price for the plan **Gold**" / "turned off …" — never "changed Custom price from No to Yes"                             |
| 9.30 | A customer request             | Send a request for more customers, then edit it, then cancel it                     | Three entries on **Customer request**, the edit naming the old→new count and the cancel reading as a void — never a raw "updated requested_count" |
| 9.31 | No raw column names anywhere   | Scroll the whole Audit Log                                                          | No `issued_at`, `voided_at`, `is_regular` or any other snake_case column ever appears in a sentence                                               |
| 9.32 | A payment says what it paid    | Void a payment taken for a **sale**                                                 | "voided a **sale payment** of **30.00 $** from **John Doe**" — not a bare "a payment of 30.00 $"                                                  |
| 9.33 | Each stream has its word       | Void a subscription payment, then a debt payment                                    | "a **subscription payment** of …" / "a **debt payment** of …" — matches the wallet filter's own wording                                           |
| 9.34 | A mixed hand-over stays plain  | Collect one payment covering a month AND a sale, then void it                       | "voided a **payment** of …" — no kind is claimed, because the cash settled more than one                                                          |

### 9b. Which service line the money was for

A customer can hold several service lines, and both bill the same months, so the month alone cannot say which one was paid. The plan rides in brackets on the record itself — **labelled** with the word "plan", never a bare name — so every action inherits it. See gotcha #141.

| #    | Scenario                        | Steps                                                                               | Expected result                                                                                                      |
| ---- | ------------------------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 9.35 | The bill names its line         | Customer with 2+ service lines → pay one month on the **Internet** line → Audit Log | "added a new **March 2026** bill (plan **Internet**) for **John Doe**" — the word "plan" says what the brackets name |
| 9.36 | The payment names it too        | Same action, the **payment** entry beside it                                        | "recorded a **subscription payment** of **20.00 $** (plan **Internet**) from **John Doe**"                           |
| 9.37 | A second payment on that month  | Pay part of a month, then pay the rest                                              | The second hand-over raises no bill, so there is only a payment entry — and it **still** names the plan              |
| 9.38 | A void says which line lost it  | Void that payment                                                                   | "voided a **subscription payment** of **20.00 $** (plan **Internet**) from **John Doe**"                             |
| 9.39 | Voiding the bill too            | Void the month bill itself                                                          | Both entries name the line — the bill's and the payment's it took down with it                                       |
| 9.40 | One hand-over, two lines        | A customer whose two lines both owe → collect all due in ONE payment                | The payment names **no** plan (honest — the cash settled two lines); each bill entry still names its own             |
| 9.41 | A sale or a debt names none     | Void a sale payment, then a debt payment                                            | No brackets at all — those bills belong to no service line                                                           |
| 9.42 | A plan line says it once        | Add a service line to a customer                                                    | "added the plan **Gold** for **John Doe**" — never "the plan **Gold** (plan **Gold**)"                               |
| 9.43 | A deleted plan drops the clause | Pay a month, then delete that plan → reopen the entry                               | The sentence reads without brackets — never "(deleted record)" and never a UUID                                      |
| 9.44 | An old row is unaffected        | Scroll to bills recorded before the line was tracked                                | No brackets; the sentence is thinner but complete                                                                    |
| 9.45 | The sheet reports no change     | Open the void entry from 9.38                                                       | "Fields changed" lists `Voided at` / `Voided by` only — **no "Plan" row**, and no "Plan: (empty) → Internet" diff    |
| 9.46 | Offline parity (native)         | Airplane mode → repeat 9.35, 9.36 and 9.38                                          | Identical sentences from the local trail                                                                             |
| 9.47 | Arabic                          | Switch to Arabic → repeat 9.35 and 9.36                                             | The plan reads in brackets on the correct side; the name is not reversed or split                                    |
