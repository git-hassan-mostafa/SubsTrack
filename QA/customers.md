...
| 1.15g | Not due yet + a backlog | Same as 1.15f but a month **older than last month** is unpaid | ONE red "Overdue" pill — owing money outranks "this month isn't due yet" |
| 1.15g2 | Last month unpaid, not late yet | `UnpaidStartRule = customer_start_day`, everything paid except **last month**, today is before the line's start day | Red **"Unpaid"** pill and **no** "Overdue" pill — the month is owed (its cell is red) but not late until the billing day (gotcha #83). On that day the pill becomes "Overdue" |
...
| 3.2c | Unpaid tab honors the tenant rule     | `UnpaidStartRule = customer_start_day`, today is before the line's start day, only **this** month unpaid | NOT in Unpaid (nothing owed yet) — they are in "Not due yet". Switch the setting to `month_start` → they move to Unpaid. With **last** month unpaid instead they are in Unpaid, not Overdue (#83) |
