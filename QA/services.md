...
6. **Writable by ANY tenant member, not just admins.** RLS (`services_modify`) copies `products_modify` verbatim, which is what lets a collector add a service from the sale form. The _screen_ sits under Admin, but the permission does not.
