...
**The sub-line does NOT add up to the headline, and that is accepted, not a bug.** The headline is net (debt payments already subtracted); the two parts are gross and omit the `custom` + `services` categories, so they can read _larger_ than the headline. A reconciling version was built and deliberately reverted at the owner's request — **don't file this as a defect.**
