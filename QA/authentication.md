...
| 6.6 | Stores cleared on logout           | Log out, log back in as a different tenant's user            | No customers / plans / users from the previous account leak in (`endSession` resets every store; the root layout repeats it when user becomes null) |
...
| 7.3  | Confirm logout                          | Tap Log Out on the dialog                                                                   | Returns to login screen, all stores reset (`endSession`)                                                                                              |
...
| 7.4b | Offline logout STICKS across a restart  | Disable internet, log out, **force-quit and re-open the app**                                | Login screen — NOT the previous user signed back in (`AUTH_STORAGE_KEY` is cleared by hand when the remote sign-out fails)                             |
| 7.4c | Logout does not sign out another device | Same user signed in on a phone and a tablet; log out on the phone                            | The tablet stays signed in (`signOut({ scope: "local" })`)                                                                                             |
...
| 7.7  | Branch preference cleared               | Tenant-wide admin picks a branch in the header → log out → log in as a **different** tenant's tenant-wide admin | Header reads "All Branches" **and** the lists really are unfiltered — customers, sales and the dashboard all show data                                 |
| 7.8  | Every store cleared, not just five      | Open Sales, Debts, Wallet, Expenses, Collections history, Audit Log and Reports → log out → log in as another organization's admin | Every one of those screens opens empty and re-fetches. No row, total or filter chip from the previous organization, **even for a moment**              |
| 7.9  | Slow load landing after logout          | Open the Products list on a throttled connection and log out **before** it finishes; log in as another organization's admin | The product list is that organization's own, and it is re-fetched (a late response must not repopulate the store or leave `loaded = true`)             |
| 7.10 | Error banner does not cross the session | Trigger a failing fetch (e.g. kill the network mid-load), log out immediately, log in as someone else | No error banner from the previous session appears on the new one                                                                                      |
| 7.11 | Create-organization button survives the reset | Enable `AllowSelfServiceSignup`, then cold-start the app signed out **on a slow / throttled connection**, and log out from a signed-in session | The login screen shows the create-organization button both times (the pre-auth `app_options` fetch must survive the store reset and the data epoch) |
