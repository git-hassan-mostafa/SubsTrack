...
describe("toParts", () => {
  it("TC-AS-01 a plain sentence is one non-bold run", () => {
    expect(toParts("nothing marked")).toEqual([
      { text: "nothing marked", bold: false },
    ]);
  });
...
describe("buildAuditSummary", () => {
  ...
  it("TC-AS-05 a month bill void names the month and the customer", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided the March 2026 bill for John Doe",
    );
  ...
  it("TC-AS-06 a single-field edit carries old and new, money formatted", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin changed price on the plan Gold from 10.00 $ to 12.00 $",
    );
  });
  ...
  it("TC-AS-07 many changed fields collapse to two plus a count", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin updated price, name and 3 other fields on the plan Gold",
    );
  });
  ...
  it("TC-AS-08 two changed fields are joined, never counted", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin updated price and months covered on the plan Gold",
    );
  });
  ...
  it("TC-AS-09 an active flag is spoken, not diffed", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin deactivated the staff member Ali Hassan",
    );
  });
  ...
  it("TC-AS-12 a write-off is its own statement, separate from a void", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin wrote off the March 2026 bill for John Doe",
    );
  });
  ...
  it("TC-AS-13 a password change never shows the value", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin changed the password for the staff member Ali Hassan",
    );
  });
  ...
  it("TC-AS-15 an unskip is the restore action on the same table", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin removed the skip on March 2026 for John Doe",
    );
  });
  ...
  it("TC-AS-16 a sale is named by its receipt number", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided the #0AB12C sale for John Doe",
    );
  });
  ...
  it("TC-AS-17 a stock entry is named by its product, which is the subject", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin changed quantity on a stock entry from 5 to 8 for Widget A",
    );
  });
  ...
  it('TC-AS-18 an old row names the TYPE, never its raw "·"-joined label', () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin changed months covered on a plan from 1 to 3",
    );
  ...
  it("TC-AS-24 cash is RECORDED, and named by its amount", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin recorded a payment of 20.00 $ from Donald Trump",
    );
  });
  ...
  it('TC-AS-25 voiding cash names the amount and says "from"', () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided a payment of 30.00 $ from Donald Trump",
    );
  });
  ...
  it("TC-AS-26 a write-off names the month once the bill carries it", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin wrote off the August 2026 bill for Donald Trump",
    );
  });
  ...
  it('TC-AS-27 a new bill is "a new March 2026 bill", never "the"', () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin added a new March 2026 bill for Donald Trump",
    );
  });
  ...
  it("TC-AS-31 a real void is still a void, not a re-open", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided the September 2026 bill for Donald Trump",
    );
  });
  ...
  it("TC-AS-19 an unknown actor is named, never left blank", () => {
    ...
    expect(sentence(e)).toBe(
      "Someone changed months covered on the plan Gold from 1 to 3",
    );
  });
  ...
  it("TC-AS-33 marking a customer as regular is spoken, not diffed", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin marked the customer John Doe as a regular customer",
    );
  });
  ...
  it("TC-AS-35 a per-customer price flag reads as a verb", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin turned on a per-customer price for the plan Gold",
    );
  });
  ...
  it("TC-AS-39 voiding a sale payment says it was for a sale", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided a sale payment of 30.00 $ from John Doe",
    );
  });
  ...
  it("TC-AS-41 a debt payment reads as a debt payment", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided a debt payment of 15.00 $ from John Doe",
    );
  });
  ...
  it("TC-AS-42 a hand-over settling several kinds stays the plain word", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided a payment of 50.00 $ from John Doe",
    );
  });
  ...
  it("TC-AS-43 an old payment row with no kind still reads", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided a payment of 30.00 $ from John Doe",
    );
  });
  ...
  it("TC-AS-44 a custom debt is named a custom debt, never a bare bill", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin added a new custom debt Router repair for John Doe",
    );
  });
  ...
  it("TC-AS-45 a custom debt void reads the same noun as its create", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin voided the custom debt Router repair for John Doe",
    );
  });
  ...
  it("TC-AS-46 a described-less custom debt falls back to its amount", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin added a new custom debt $20.00 for John Doe",
    );
  });
  ...
  it("TC-AS-47 a month bill keeps the bare word — its month already names it", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin added a new March 2026 bill for John Doe",
    );
  });
  ...
  it("TC-AS-52 a month bill names the service line it belongs to", () => {
    ...
    expect(sentence(e)).toBe(
      "Super Admin added a new March 2026 bill (plan Gold) for John Doe",
    );
    expect(bolded(e)).toEqual([
      "Super Admin",
      "March 2026",
      "Gold",
      "John Doe",
