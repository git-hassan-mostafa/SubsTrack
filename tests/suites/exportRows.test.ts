...
describe('flattening', () => {
  it('keeps the name behind a dropped id', () => {
    const out = flattenRow({ planId: 'p-1', plan: { id: 'p-1', name: 'Gold' } });
...
describe('columns', () => {
  ...
  it('takes the union of the rows keys, first seen first', () => {
    const fields = fieldsOf([{ a: 1, b: 2 }, { a: 3, c: 4 }]);
...
describe('loading every page', () => {
  ...
  function pagedList(total: number, pageSize = 30) {
    ...
    return {
      ...
      fetchMore: async () => {
        ...
        rows.push(...Array.from({ length: Math.min(pageSize, total - start) }, (_, i) => start + i));
        page += 1;
      },
    };
  }
  ...
  it('does nothing when there is nothing more to load', async () => {
    ...
    const all = await loadAllPages(() => [1, 2], () => false, fetchMore);
  ...
  it('gives up when a page adds nothing', async () => {
    ...
    const all = await loadAllPages(() => [1], () => true, fetchMore);
