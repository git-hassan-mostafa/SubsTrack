...
function paidBy(id: string, amount: number, chargeId: string, receivedAt: string) {
  return collection({
    id,
    amount,
    receivedAt,
...
describe('buildBillInvoiceText', () => {
  ...
  it('counts only the slice that reached THIS bill', () => {
    const split = collection({
      ...
      amount: 50,
      items: [
        collectionItem({ collectionId: 'col-1', chargeId: 'chg-1', amount: 20 }),
        collectionItem({ collectionId: 'col-1', chargeId: 'chg-9', amount: 30 }),
      ],
    });
