...
describe("currency", () => {
  ...
  it("TC-CU-05 snapshotCurrency pins the row`s rate onto today`s currency", () => {
    const pinned = snapshotCurrency(
      { currencyId: LBP.id, ratePerUsdSnapshot: 60000 },
      [LBP],
    );
    expect(pinned!.ratePerUsd).toBe(60000);
    expect(pinned!.code).toBe("LBP");
    expect(
      snapshotCurrency({ currencyId: null, ratePerUsdSnapshot: 1 }, [LBP]),
    ).toBeNull();
  });
  ...
  it("TC-CU-07 formatPaidFraction prints the label once, bidi-isolated", () => {
    expect(formatPaidFraction(20, 50, null, null)).toBe(
      "\u2068" + "20.00/$50.00" + "\u2069",
    );
  });
...
describe("sumByMonth (the money-in section headers)", () => {
  ...
  it("TC-DT-11 a zero or missing rate never divides by zero", () => {
    expect(
      sumByMonth([
        {
          received_at: "2026-01-10T09:00:00.000Z",
          amount: 20,
          rate_per_usd_snapshot: 0,
        },
      ])["2026-01"],
    ).toBe(20);
  });
