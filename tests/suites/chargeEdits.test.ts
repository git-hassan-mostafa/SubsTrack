...
const balanceOf = async (id: string) =>
  (await fakeChargeRepository.balances([id]))[0];
...
describe("addManualCharge", () => {
  ...
  it("TC-CH-01 refuses a non-positive or non-finite amount", async () => {
    for (const amount of [0, -1, NaN]) {
      await expect(
        chargeService.addManualCharge({ ...base, amount }),
  ...
  it("TC-CH-03 refuses a rate snapshot that is not positive", async () => {
    await expect(
      chargeService.addManualCharge({ ...base, ratePerUsdSnapshot: 0 }),
...
describe("voidChargeWithPayments (the bill AND its cash)", () => {
  it("TC-CH-20 voids every live hand-over on the bill, then the bill", async () => {
    ...
    expect(
      store.collections.find((c) => c.id === a.id)!.voided_at,
    ).not.toBeNull();
    expect(
      store.collections.find((c) => c.id === b.id)!.voided_at,
    ).not.toBeNull();
    expect(store.charge(chg.id)!.voided_at).not.toBeNull();
  });

  it("TC-CH-21 a SHARED hand-over is voided whole - the other bill loses its money too", async () => {
    const jan = store.seedCharge({
      id: "jan",
      amount: 20,
      billing_month: "2026-01-01",
    });
    const feb = store.seedCharge({
      id: "feb",
      amount: 20,
  ...
  it("TC-CH-22 paymentIdsForCharges de-dupes one hand-over covering many bills", async () => {
    ...
    expect(await chargeService.paymentIdsForCharges(["jan", "feb"])).toEqual([
      col.id,
    ]);
  });
  ...
  it("TC-CH-23 a voided hand-over is not re-voided", async () => {
    const chg = store.seedCharge({ amount: 20 });
    const col = store.seedCollection(chg.id, 20, {
      voided_at: "2026-02-05T00:00:00.000Z",
    });
    await chargeService.voidChargeWithPayments(chg.id, "user-2", "again");
    expect(
      store.collections.find((c) => c.id === col.id)!.voided_by,
    ).toBeNull();
  });
});
...
describe("writeOff (real, but lost)", () => {
  it("TC-CH-30 refuses a voided bill and a second write-off", async () => {
    const voided = store.seedCharge({
      amount: 20,
      voided_at: "2026-02-01T00:00:00.000Z",
    });
    await expect(
      chargeService.writeOff(voided.id, "user-1", null),
    ...
    await expect(
      chargeService.writeOff(lost.id, "user-1", null),
...
describe("writeOffMany (give up on everything one customer owes)", () => {
  ...
  it("TC-CH-39b an empty list writes nothing", async () => {
    await expect(
      chargeService.writeOffMany([], "user-1", null),
    ).resolves.toEqual([]);
  });
});
...
describe("updateManualCharge", () => {
  it("TC-CH-40 refuses a non-positive amount", async () => {
    ...
    await expect(
      chargeService.updateManualCharge(chg.id, { amount: 0 }),
    ).rejects.toThrow(/errors\.debt_amount_positive/);
  });
  ...
  it("TC-CH-42 REGRESSION: it must not price a bill below what was collected", async () => {
    ...
    await expect(
      chargeService.updateManualCharge(chg.id, { amount: 20 }),
    ).rejects.toThrow();
  });
  ...
  it("TC-CH-43 REGRESSION: it must not edit a voided or written-off bill", async () => {
    ...
    await expect(
      chargeService.updateManualCharge(voided.id, { amount: 20 }),
    ).rejects.toThrow();
  });
  ...
  it("TC-CH-46 the amount may fall exactly to what was collected, never below", async () => {
    ...
    await expect(
      chargeService.updateManualCharge(chg.id, { amount: 19.99 }),
