...
describe("groupOwedByCurrency", () => {
  it("TC-XC-01 splits the pool into one group per currency", () => {
    ...
    expect(groups.map((g) => g.currencyId).sort()).toEqual(
      [LBP.id, null].sort(),
    );
  });
...
describe("planCollection", () => {
  ...
  it("TC-XC-09 an unticked bill drops out of its own group", () => {
    const older = openItem({
      chargeId: "chg-a",
      amount: 20,
      dueDate: "2026-01-01",
    });
    const newer = openItem({
      chargeId: "chg-b",
      amount: 20,
      dueDate: "2026-03-01",
    });
