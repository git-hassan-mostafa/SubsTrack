...
describe("sale list patches", () => {
  it("TC-SP-01 a new sale goes to the top (every list is newest first)", () => {
    ...
    expect(addSale(existing, sale({ id: "new" })).map((s) => s.id)).toEqual([
      "new",
      "old",
    ]);
  });
  ...
  it("TC-SP-04 a void removes the row", () => {
    expect(
      removeSales([sale({ id: "a" }), sale({ id: "b" })], ["a"]).map(
        (s) => s.id,
      ),
    ).toEqual(["b"]);
  });
  ...
  it("TC-SP-04b a void drops the row from a list that hides voided sales", () => {
    ...
    expect(applyVoidedSales(items, [voided], false).map((s) => s.id)).toEqual([
      "b",
    ]);
  });
  ...
  it("TC-SP-04d a whole selection is voided in one pass, order kept", () => {
    ...
    const voided = ["a", "c"].map((id) =>
      sale({ id, voidedAt: "2026-02-02T00:00:00.000Z" }),
    );
    expect(applyVoidedSales(items, voided, false).map((s) => s.id)).toEqual([
      "b",
    ]);
    expect(
      applyVoidedSales(items, voided, true)
  ...
  it("TC-SP-05 money lands only on the sales the hand-over actually names", () => {
    const items = [
      sale({ id: "a", chargeId: "chg-a" }),
      sale({ id: "b", chargeId: "chg-b" }),
    ];
    const col = collection({
      items: [collectionItem({ chargeId: "chg-a", amount: 12 })],
    });
    expect(applyCollectionToSales(items, col).map((s) => s.amountPaid)).toEqual(
      [12, 0],
    );
  });
  ...
  it("TC-SP-08 saleUsd converts at the sale`s OWN frozen rate", () => {
    expect(
      saleUsd(sale({ totalAmount: 180000, ratePerUsdSnapshot: 90000 })),
    ).toBe(2);
  });
});
...
describe("sale lines", () => {
  it("TC-SP-10 a service line is always one unit, whatever it costs", () => {
    ...
    expect(
      lineQuantity({ kind: "product", product, quantity: 4, unitAmount: 5 }),
    ).toBe(4);
  });
...
describe("sharedBillsOf", () => {
    ...
    const col = collection({
      items: [
        collectionItem({
          chargeId: "jan",
          amount: 20,
          charge: charge({ id: "jan", billingMonth: "2026-01-01" }),
        }),
        collectionItem({
          chargeId: "feb",
          amount: 20,
          charge: charge({ id: "feb", billingMonth: "2026-02-01" }),
        }),
      ],
    });
  ...
  it("TC-SS-02 a hand-over that settled only this bill has nothing to warn about", () => {
    const col = collection({
      items: [collectionItem({ chargeId: "jan", amount: 20 })],
    });
  ...
  it("TC-SS-03 the amount carries the PARENT hand-over`s currency and rate", () => {
    ...
    expect(sharedBillsOf(col, "jan", t)[0].snapshot).toEqual({
      currencyId: LBP.id,
      ratePerUsdSnapshot: 90000,
    });
  });
  ...
  it("TC-SS-04 across many hand-overs, the same bill merges — but never across currencies", () => {
    const usd = collection({
      id: "c1",
      items: [collectionItem({ id: "a", chargeId: "feb", amount: 5 })],
    });
    const usd2 = collection({
      id: "c2",
      items: [collectionItem({ id: "b", chargeId: "feb", amount: 7 })],
    });
    ...
    expect(merged.find((b) => b.snapshot.currencyId === LBP.id)!.amount).toBe(
      90000,
    );
  });
  ...
  it("TC-SS-05 a line with no loaded charge still names something", () => {
    const col = collection({
      items: [collectionItem({ chargeId: "x", amount: 5 })],
    });
    expect(sharedBillsOf(col, "jan", t)[0].label).toBe(
      "ledger.shared_bill_fallback",
    );
  });
});
