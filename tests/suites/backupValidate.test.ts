...
describe('TC-BK-02 the tenant guard', () => {
  it('refuses a file whose envelope names another organization', () => {
    const file = makeFile({}, { tenant: { id: OTHER_TENANT, code: 'X', name: 'Other Co' } });
  ...
  it('exempts the global app_options table from the tenant rule', () => {
    const file = makeFile({ app_options: [{ id: 'o1', key: 'LiraRate', value: '90000' }] });
...
describe('TC-BK-06 file shape', () => {
  it('refuses a file that is not a SubsTrack backup', () => {
    expect(problemOf({ format: 'something-else', version: 1 }).code).toBe('wrong_format');
  });
  ...
  it('refuses a nested object as a column value', () => {
    const file = makeFile({
      customers: [
        { id: 'c1', tenant_id: TENANT, name: { evil: true } } as unknown as BackupRow,
      ],
    });
    expect(problemOf(file)).toMatchObject({
      code: 'bad_value',
  ...
  it('drops an unknown column and warns instead of failing', () => {
    ...
    expect(result.backup.tables.customers[0]).not.toHaveProperty('legacy_column');
  });
});
