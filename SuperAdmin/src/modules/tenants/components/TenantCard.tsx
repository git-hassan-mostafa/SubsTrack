...
export function TenantCard({ tenant, saasTier, onEdit, onDelete }: TenantCardProps) {
  return (
    <View style={styles.card}>
      <Pressable style={styles.body} onPress={() => onEdit(tenant)}>
        <View style={styles.header}>
          <Text style={styles.name}>{tenant.name}</Text>
          <View style={[styles.badge, tenant.active ? styles.badgeActive : styles.badgeInactive]}>
            <Text style={[styles.badgeText, tenant.active ? styles.badgeTextActive : styles.badgeTextInactive]}>
      ...
      <Pressable onPress={() => onDelete(tenant)} style={styles.deleteBtn} hitSlop={8}>
