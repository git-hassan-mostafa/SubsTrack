...
export class TenantService {
  ...
  async createTenant(data: CreateTenantInput): Promise<Tenant> {
    ...
    if (!data.adminEmail.includes('@')) throw new Error('Admin email is invalid');
    if (data.adminPassword.length < 8) throw new Error('Password must be at least 8 characters');

    const row = await this.repository.create({ name: data.name.trim() });
    ...
    const username = data.adminEmail.trim().split('@')[0];
