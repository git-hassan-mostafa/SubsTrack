...
**Windowed tables.** `TableSpec.pullDays` keeps only a rolling local window (`audit_logs`: 30, `AUDIT_LOCAL_DAYS`) — a device does not need a year of trail to answer "what happened lately", and the full history is one online read away. The pull adds `.gte('occurred_at', cutoff)`, and `pruneWindowedTables(db)` deletes older local rows. **The prune is guarded by `_dirty = 0`**: an un-pushed row is the ONLY copy in existence, so age must never delete it. It runs after each complete sync cycle **and** once at bootstrap, so a device offline for months still prunes.
