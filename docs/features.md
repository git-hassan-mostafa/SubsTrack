...
| **Multi-select of records already collected** — month grid, `PaymentsPanel`, `SalesPanel` + `CustomerSalesListScreen` | a **Send invoice on WhatsApp** action (receipt icon) — **one** receipt covering the whole selection |
...
**The entry card is two lines, and that is a constraint, not a coincidence.** `AuditEntryCard` shows **record type + `subject` + the action pill**, then **staff · when · what moved**. It used to also list the changed field names as chips, which pushed every row to three or four lines — a long trail became a wall — while still saying less than one tap does. So line 2 ends with a **phrase, not a chip row**: one changed field is named (`Amount paid`), several are only counted (`3 fields changed`), and a create/delete says nothing there (the whole row changed; the type and record already cover it). The action is carried by **colour + icon + pill**, never prose, so the list scans by shape before it is read. When the record label would repeat the subject (a `customers` entry, where both are the same name) the label is dropped — and the detail sheet mirrors that, showing its Customer row only when the subject isn't already the Record row.
