...
Everything the form owns can change: products, quantities, unit prices, the sale currency, the customer, the amount collected and the notes. What identifies the sale cannot: `id`, `tenant_id`, `sold_at`, and the original `recorded_by_user_id` (who made the correction is in the audit trail, not on the row). Five rules make it safe:
...
- **Manual** — `ProductService.adjustStock` appends a single `restock` / `adjustment` row. A row is never deleted, and a `'sale'` row is never touched by hand; a **manual** row can be corrected in place — see [Editing a stock entry](#editing-a-stock-entry) for when that is right and when another movement is.
...
**A REMOVAL may carry a cost too, and then it is a credit.** `amount = quantity_delta × unit_cost`, so a costed negative `'adjustment'` is a negative expense — and it is the **only** way an over-stated stock expense comes back down, since a derived row can't be voided. The sign is not what decides; the cost is:

| Remove 2 units… | Meaning | Expenses |
| --- | --- | --- |
| with the cost field **empty** | damaged, lost, stolen — the money **was** spent | unchanged |
| with a **cost** typed | wrong quantity entered, or returned to the supplier — the money comes back | drops by 2 × cost |

The app cannot tell those apart from the delta, so staff say which: the stock sheet keeps its **Cost per unit** field in Remove mode but **clears** it (Add mode pre-fills the product's cost price), captions it "money back", and says the credit in green ("Takes $X off Expenses"). Cash basis still rules — the credit lands in the month it was **recorded**, so correcting a July delivery in August leaves July whole. Cost is typed in three places: the product form's **Cost price** field (the default, plus the opening stock's cost on create), the stock sheet's **Cost per unit** / **Total cost** pair in either mode (see below), and the **batch restock** sheet, where one **delivery currency** is picked for the whole save and each picked row opens a cost line seeded from its product's cost price, converted at the live rate (the `SaleItemsEditor` rule — changing the delivery currency re-prices every row). The stock history shows a costed row's money ("Cost: $X" / green "Money back: $X"), so which rows moved Expenses is visible.

**Per unit or per delivery — both are typeable, and each fills the other.** A supplier invoice states one or the other ("4.50 each", "45 for the lot"), so the stock sheet puts **Cost per unit** and **Total cost** side by side in both directions: typing either one recomputes the other from the quantity (`total = unit × qty`, `unit = total ÷ qty`). Only **`unit_cost`** is ever saved — the total is a way of entering it, not a column — so the derived unit keeps **8 decimals** (what `stock_movements.unit_cost` stores): rounding 100 ÷ 3 to 33.33 would make the recorded expense 99.99 and disagree with the invoice that was typed. **The last field staff typed is the anchor**, so changing the quantity afterwards recomputes the *other* one and never overwrites what they entered — typed a 45 total, then fixed 10 units to 12, and the unit becomes 3.75 while the total stays 45. Everything else keeps the per-unit field as the source of truth: the Add/Remove toggle, an abandoned edit, and picking Edit on a row all reset the anchor to "unit". One currency for both — the picker sits on the per-unit input and the total is locked to it, since a movement stores one currency.

#### Editing a stock entry

A **manual** movement can be corrected in place — `ProductService.updateMovement` → `IProductRepository.updateMovement`, reached from the history row's 3-dot menu → **Edit entry**. There are now three doors into "the stock number is wrong" (the third is [Reverting a stock entry](#reverting-a-stock-entry), which follows the edit's month rule), and the two that carry money are **not** interchangeable:

| | **Edit the row** | **Add a costed removal** |
| --- | --- | --- |
| What happened | the entry was **written** wrong (12 typed for a 10-unit delivery, a cost of 0.50 the invoice says was 0.45) | something **really happened later** (2 went back to the supplier) |
| The history says | 10 arrived | 12 arrived, then 2 went back |
| The month that moves | the entry's **own** month — July stays $5.00 | the month it was **recorded** — July keeps $6.00, August gets −$1.00 |

Both are needed: an edit is the only thing that can fix a past month, and a removal is the only thing that can tell the truth about a later event. One question decides: *did I write it down wrong, or did something really happen?*
...
**UI.** One form does both jobs, like `SaleFormSheet`: picking Edit fills the sheet's quantity / cost / note from the row, swaps the Add/Remove toggle for an "Editing this entry" banner (direction locked, with a Cancel ✕ and a one-line note on when an edit is the wrong tool), and turns the button into "Save Changes". The tapped row sits far below the form, so picking Edit also **scrolls the banner into view** (`useSheetScrollTo(y)`, published by `FormSheet`; the offset is the banner’s own `onLayout` `y`, and it is keyed on the row so moving from one edit to another re-fires it) — otherwise the action looks like it did nothing. Saving **keeps the sheet open** and reloads the history — a correction is only believable next to the rows it fixed — and resets the form to its first-render state so the unsaved-changes guard stays quiet.
...
**The month follows the EDIT rule, not the costed-removal one.** Reverting says the entry was never real, so the money comes off the month the entry belongs to: a July delivery reverted in August leaves August untouched and drops July's expense. A costed removal is the opposite door and still the only way to record stock that really went back later (see [Stock](#stock) → cost, gotchas #94 / #96).
...
| `stock` | **Derived** at read time from `stock_movements` — costed, non-voided, non-`'sale'` rows; `amount = quantity_delta × unit_cost`, so a costed **removal** is a negative amount (money back) |

**A restock never writes an expense row.** Deriving it means correcting the stock corrects the expense, with no second insert inside the offline restock transaction, no drift on a void, and no orphan when a hard-deleted product takes its ledger with it. The cost of that choice is that a derived row **cannot be voided** (`ExpenseItem.canVoid` is false; its 3-dot offers "Open product") — a wrong cost is fixed the way every stock mistake is, with another movement, and that movement is what carries the money back: **a costed removal is a credit** (see [Stock](#stock) → cost, and gotcha #94). Row ids are prefixed (`exp:` / `stock:`) so the two sources can never collide.
...
- **Debt history** — a **clock icon** (`time-outline`) on the net-total summary card opens `DebtHistorySheet`: a **read-only, branch-wide activity log** merging every outstanding debt **and** every debt payment into one newest-first list, bucketed into date sections (Today / This Week / This Month / `<Month> <Year>`) via the shared `groupByMonth` + `MonthSectionHeader` — the same grouping the Payments/Sales tabs use. It reuses the slice's already-loaded `items` + `payments` (no re-fetch) and reuses `DebtItemCard` / `DebtPaymentCard` (customer name shown, no row actions). This restores the removed Debts / Payments sub-tabs as a single combined, read-only sheet. Two rules it must keep (gotcha #74):
