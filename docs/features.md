...
- **Voiding, skipping, editing an amount and viewing a receipt are unaffected** — the rule is about recording new money only.

One implementation, two layers: `blockingUnpaidMonths()` in [`utils/payOrder.ts`](../SubsTrack/src/modules/customer/customer-payments/utils/payOrder.ts) decides, `PaymentService.assertPayableInOrder` enforces it inside every payment-slice create action, and the UI reads the same helper through the slice's `unpaidMonthsByLine` (per line, all years) and `CustomerStatus.overdueLineIds` (customer list). See gotcha #77.
