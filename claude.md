...
- **Skipped months** (`skipped_months` table) are months a line is **not expected to pay** — never unpaid, never overdue, and **never payable** (the UI offers "Unskip" instead, and a multi-month block covering one is refused). One row per `(customer_plan_id, billing_month)` with a `skipped` boolean toggle; unskip flips it to `false` and keeps the row so the change syncs. Any user can skip; an optional note says why. A customer whose **every** started line is skipped this month owes nothing **this month**, so the list shows a slate **"Skipped"** badge and the Unpaid tab drops them — unless an older month is still unpaid, which reads as **"Overdue"** (a skip excuses its own month, never a backlog). Full behavior in [docs/features.md](docs/features.md) → Skipped Months.
- **Months are settled OLDEST FIRST.** A month cannot be paid while an **earlier** month of the same service line is still unpaid — the collector must clear the backlog in order, so a customer can never have a paid March sitting on top of an unpaid January. The rule is one pure helper, `blockingUnpaidMonths()` in `customer-payments/utils/payOrder.ts`, fed by `PaymentService.unpaidBillingMonths(line, …)` (which walks `buildMonthGrid` across **every** year from the line's start — a backlog in a previous year still blocks, and the viewed grid can't see it). Enforced in two layers: `PaymentService.assertPayableInOrder` runs inside every payment slice create action (`createPayment`, `createPayments`, `createMultiMonthPayment(s)`), and the UI gates ahead of it (cell tap, quick-pay menu, grid multi-select, quick-pay deep link, the form's amber banner, and customer-list quick pay via the new `CustomerStatus.overdueLineIds`). **Months inside the same write never block it** — selecting Jan+Feb+Mar and paying them together is allowed; paying only Mar is not. Skipped and partially-paid months are not "unpaid", so they never block. The slice publishes `unpaidMonthsByLine` next to `monthGridsByLine` (one pass in `buildGridsFor`).
- **Voids run NEWEST FIRST, and a paid line's start date is FROZEN** — the two other doors into "a paid month sitting on an unpaid one". Same shape as the pay rule: one pure helper `blockingPaidMonths()` (the mirror of `blockingUnpaidMonths`, in `payOrder.ts`), one service guard `PaymentService.assertVoidableInOrder` **inside** `voidPayment` / `voidPayments` / `voidCurrentMonth` — it resolves the rows from the ids itself (`repository.findByIds`), so the Transactions → Payments tab, which never loads a grid, is covered too — and a UI pre-gate in `CustomerPaymentPanel` (detail sheet, cell menu, multi-select) fed by the new `paidMonthsByLine`. Months inside the same void never block each other, and a partially-paid later month still blocks. The start-date lock lives in `CustomerPlanService.syncLines` (`assertStartDatesUnlocked`, checked only for lines whose date actually changed) with the form's date input disabled for any line in `repository.findPaidLineIds(customerId)` — non-voided, `amount_paid > 0`, which is **not** the same as the delete prompt's `countPayments` (that one counts voided rows on purpose). Together with the pay rule, a paid month sitting on an unpaid one is unreachable for new data — only legacy rows predating these guards, or an unskipped old month, can still produce one, and the badge reports it as **"Overdue"** (never "✓ Paid", which now means "owes nothing"). See gotcha #79.
- **Multiple plans per customer:** a customer holds 1..N service lines (`customer_plans`), each its own grid + independent payments. **The service line owns the ONLY start date** — `customers` has no `start_date` at all (a customer starts when its first line does). Lines are added / changed / removed from the **customer form's inline Plans editor** (`customerPlans.syncLines`); the payment panel's line selector is **view-only**. Status is **aggregated across a customer's active lines** by `PaymentService.buildCustomerStatus`: paid only when **every** line owes nothing across **all** its required months (a partial payment counts as covered), overdue if any active line has an earlier unpaid month. A customer with **2+ in-play plans where some owe nothing and some still owe** shows an amber **"N/M plans paid"** badge (`CustomerStatus.planCount`), so a partly-paid account never looks fully unpaid. Customer-list "Collect all due" pays every eligible fixed-price line for the current month at once — **skipping any line with an older unpaid month**, which must be collected from the detail grid instead. Full detail in [docs/features.md](docs/features.md) → Multiple Plans per Customer.
...
| `overdueLineIds` | lines with an EARLIER unpaid month — quick pay must skip them too (months are settled oldest-first); the per-line detail behind `overdue` |
