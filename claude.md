...
(`products` + `sales` tables also exist — see [docs/features.md](docs/features.md) → Products & One-Off Sales. `sales` gained `amount_paid` — partial sales leave a debt. `custom_debts` + `debt_payments` back the Debts tab.)
