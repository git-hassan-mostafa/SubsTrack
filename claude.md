...
| `audit_logs` | `id`, `tenant_id`, `branch_id` (denormalized from the row or its parent; NULL = tenant-wide — **no FK on purpose**: `ON DELETE SET NULL` would blank the trail when a branch is deleted), `table_name`, `record_id`, `action` (`create`\|`update`\|`delete`\|`void`\|`restore`), `before_data` / `after_data` / `changed` (JSONB — an **edit stores only the changed columns**), `label` (frozen one-liner), `subject` (frozen customer name — who the record belongs to), `actor_user_id`, `actor_username` (snapshot, survives user deletion), `occurred_at` (**device** clock = when staff acted, NOT sync time), `created_at`, `updated_at`. **Append-only, written by the app — never by a trigger** (a trigger would stamp the sync moment and the syncing session, and an offline device would keep no history). RLS: SELECT **admins only** (branch-aware), INSERT **every tenant member**, and **no UPDATE/DELETE policy** (service_role only). See [docs/features.md](docs/features.md) → Audit Trail. |
