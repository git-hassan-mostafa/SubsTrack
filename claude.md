...
DashboardMetrics { totalCustomers, activeCustomers, monthlyRevenue, subscriptionRevenue, salesRevenue, unpaidThisMonth, totalUsers, totalPlans, totalOutstandingBalance, newCustomersThisMonth, cancelledThisMonth, paymentsCollectedCount, salesCount, prevMonthRevenue, revenueTrend /*RevenuePoint[] — 6 months ending on the current month; the chart can page further back/forward via `DashboardService.getRevenueTrend(anchorYear, anchorMonth)` + the dashboard slice's `trend`/`trendAnchor`/`navigateTrend`, capped at the current month*/ }
