...
- Web/desktop width is capped via `<ResponsiveContainer>` (`src/shared/components/ResponsiveContainer.tsx`) — wraps each screen's body (and each page-sheet form) in a centered, max-width column. It's a no-op on phones (always narrower than the cap), so phone layout is unaffected; it only kicks in on wide viewports. Centered dialogs (`ConfirmDialog`, `UpgradePromptModal`) cap themselves inline with a `max-w-*` class instead.
- Transient tap-outside popups share one shell: `<BottomSheetScaffold>` (`src/shared/components/BottomSheetScaffold.tsx`) — a **bottom sheet**: slides up from the bottom edge, pinned to the bottom, centered horizontally (capped `maxWidth: 512` on wide viewports), with a drag-handle line at the top and bottom safe-area padding. The user can **drag the sheet down to dismiss** (past ~25% of its height, or a fast downward flick); the slide + drag are built on **Reanimated + a gesture-handler `Pan` gesture** (not RN `animationType="slide"`, not the Gorhom library), and the backdrop only fades in place. Structural layout uses explicit `style` (NativeWind `className` doesn't reliably resolve flex/align/width on Animated views). All the pickers/menus (`Dropdown`/`DropdownModal`, `DatePickerInput`, `CurrencyInput`, `AsyncEntityPicker`, `ActionMenu`) render their content inside it; a caller needing keyboard avoidance passes a `wrap` (a `KeyboardAvoidingView`). Deliberately does NOT use `useWebBackDismiss` (kept out of the browser history stack — closes by tapping the backdrop). **Confirmations / message dialogs (`ConfirmDialog`, `UpgradePromptModal`) stay centered** — they are not bottom sheets. Build any new picker/menu popup on this shell. See gotcha #45.
- Page sheets share one shell: `<SheetModal>` (`src/shared/components/SheetModal.tsx`) — RN `Modal` + the standard `slide`/`pageSheet` chrome + `onDismiss` wired to native hardware-back AND browser Back on web. Build new sheets on it. On web, browser **Back closes the topmost open modal** instead of changing the route (mirrors Android hardware-back) via the `useWebBackDismiss(active, onDismiss)` hook. Scope is deliberately narrow: **only form sheets + the dialogs that stack on them (`ConfirmDialog`, `UpgradePromptModal`) use it**; transient tap-outside popups (dropdowns, pickers, `ActionMenu`) close by tapping their backdrop and are kept out of the history stack. See gotcha #44.
- `<PageHeader>` (`src/shared/components/PageHeader.tsx`) is on every screen and carries a top-right **3-dot quick-actions menu** — app-wide "quick add" shortcuts (Add customer / Record sale / Add custom debt / Record debt payment). It's a UI-only global-store seam mirroring `confirm`: the menu flips the generic `ui` slice (`src/state/slices/ui/`, the home for ephemeral cross-screen UI state — distinct from the persisted `uiPrefStore`) and the sheets are hosted once by `QuickActionSheets` (`src/modules/quick-actions/`) in `app/(app)/_layout.tsx`. Pass `hideQuickActions` to suppress it. Detail in [docs/features.md](docs/features.md) → Debts.
