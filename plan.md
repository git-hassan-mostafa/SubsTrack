...
All cells except BEFORE_START show a 3-dot menu in the corner with: **Open** (same as tapping), **Pay** (Quick Pay for the tapped month — on UNPAID or FUTURE/prepay with a fixed-price plan; multi-month plans confirm first, custom-price/no-plan fall back to the form), and **Void** (only when a non-voided payment exists). BEFORE_START cells stay tap-only.
...
| B — Optional override | Plan exists, is_custom_price = false, user wants to override | Radio: "Plan price ($X)" or "Custom". When "Custom" is selected, a Full/Partial toggle appears. |
| C — Fully custom | Plan is_custom_price = true, or customer has no plan | Full/Partial toggle shown. Full mode: single amount input. Partial mode: Amount Due + Amount Paid inputs. |
| D — Multi-month | Plan exists, is_custom_price = false, duration_months > 1 | Pre-filled bundle price (locked). Full/Partial toggle shown. Partial mode: Amount Paid input only (Amount Due = plan.price). |
...
When a customer pays only part of their due amount, staff selects "Partial payment" mode and enters:
- `amount_due`: what was owed (snapshot, never changes after recording)
- `amount_paid`: what was actually collected (can be less than amount_due)
...
Store: billing_month, amount_due (snapshot), amount_paid (snapshot), balance (generated by DB), customer_id, plan_id, tenant_id, received_by_user_id, notes (optional).
...
- **Stock.** Every product carries a quantity and can be **out of stock**. Stock on hand is computed at runtime as `SUM(stock_movements.quantity_delta)` over non-voided rows — never a counter column (an offline counter would be clobbered by whole-row sync). A SHARED product has one pool across all branches.
  ...
  - A **batch restock sheet** covers a whole arriving delivery in one save: a searchable list of active products, each with a quantity stepper, plus one shared note. It writes one `'restock'` movement per product, so each product's history stays as detailed as the one-at-a-time path. Opened from the "Restock" button on the products screen or "Batch Restock" in the quick-actions menu (admin-only).
...
1. NEVER derive payment amount from plan.price after a payment has been recorded. amount_due and amount_paid are snapshots stored at insert time. balance is always computed from them.
